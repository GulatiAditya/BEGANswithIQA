...
class GANTrainer:
	def __init__(self, gen, disc, gan, data,loss, kLambda = .001, logEpochOutput = True, saveModelFrequency = 5, 
                 sampleSwatch = True, saveSampleSwatch = True):
		'''
			...
			for b in range(nb_batch_per_epoch):
				...
				if(self.sampleSwatch and b % (nb_batch_per_epoch / 2) == 0):
					if(self.saveSampleSwatch):
						genName = '/generatorSample{}_{}_{}.png'.format(self.loss,e, int(b/nb_batch_per_epoch/2))
						discName = '/discriminatorSample_{}_{}.png'.format(self.loss,e, int(b/nb_batch_per_epoch/2))
					else:
